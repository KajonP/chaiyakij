    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-12 align-self-end">
                <h2>ยอดขายตาม sales</h2>
            </div>
            <div class="col-12">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item">
                            <a href="{{ route('report_transaction') }}">รายงาน</a>
                        </li>
                        <li class="breadcrumb-item active" aria-current="page">ยอดขายตาม sales</li>
                    </ol>
                </nav>
            </div>
        </div>
...
<div style="padding-top: 50px; padding-right: 30px;padding-left:40px;">
	รวมราคา 99,999 บาท
</div>
...
<script type="text/javascript">
    ...
    serverSide: true,
    ajax: {
        url: "{{ route('api:report:per_sale:list') }}",
        method: "GET",
        data: function(d) {
            d._token = "{{ csrf_token() }}";
            d.search = $("#search").val(); alert(d.search);
            d.date_start = ds; alert(d.date_start);
            d.date_end = de; alert(d.date_end);
    ...
    columns: [
        ...
        { className: "", data: "main_name" },
        { className: "", data: "type_name" },
        { className: "", data: "product_size_name_text" },
        {
        className: "text-center",
        data: function(a) {
            return numeral(a.price).format("0,0");
        }
        },
        {
        className: "text-center",
        data: function(a) {
            return numeral(a.qty).format("0,0");
        }
        },
        {
        className: "text-center",
        data: function(a) {
            return numeral(a.total_price).format("0,0");
        }
        },
    ]
    });
